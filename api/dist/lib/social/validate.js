...
const isJpeg = (m) => /^image\/jpe?g$/i.test(m.mimeType ?? '');
/** Hashtags and mentions, counted the way a platform counts them. */
export function countTags(text) {
    return {
...
function instagram(post, media, caption) {
    const out = [];
    const err = (field, message, rule) => out.push({ network: 'instagram', field, message, rule, severity: 'error' });
    ...
    if (hashtags > IG_HASHTAG_MAX)
        err('caption', `Instagram allows ${IG_HASHTAG_MAX} hashtags and this has ${hashtags}.`, 'X-LIM-01');
    if (mentions > IG_MENTION_MAX)
        err('caption', `Instagram allows ${IG_MENTION_MAX} @ mentions and this has ${mentions}.`, 'X-LIM-01');
    ...
    for (const m of media) {
        ...
        if (isImage(m)) {
            ...
            if (!isJpeg(m))
                err('media', `${where} must be a JPEG for Instagram. Klippy converts it when it can.`, 'X-LIM-07');
            if (m.bytes != null && m.bytes > IG_IMAGE_BYTES_MAX)
                err('media', `${where} is over 8 MB, which Instagram refuses.`, 'X-LIM-07');
            if (m.width != null) {
                if (m.width < IG_IMAGE_MIN_WIDTH)
                    err('media', `${where} is ${m.width} px wide. Instagram needs at least ${IG_IMAGE_MIN_WIDTH}.`, 'X-LIM-07');
...
function facebook(post, media, caption) {
    const out = [];
    const err = (field, message, rule) => out.push({ network: 'facebook', field, message, rule, severity: 'error' });
...
function linkedin(post, media, caption) {
    const out = [];
    const err = (field, message, rule) => out.push({ network: 'linkedin', field, message, rule, severity: 'error' });
    if (!caption && !media.length) {
 ...
 */
export function validatePost(post, media, networks, overrides = {}) {
    const sorted = [...media].sort((a, b) => a.position - b.position);
    const issues = [];
    for (const n of networks) {
        const caption = captionFor(post, overrides[n]);
        if (n === 'instagram')
            issues.push(...instagram(post, sorted, caption));
        if (n === 'facebook')
            issues.push(...facebook(post, sorted, caption));
        if (n === 'linkedin')
            issues.push(...linkedin(post, sorted, caption));
    }
    if (!networks.length) {
        issues.push({
