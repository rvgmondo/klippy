...
export async function paymentRoutes(app) {
    ...
    app.get('/api/v1/payfast/mode', { preHandler: app.requireAuth }, async (req, reply) => {
        ...
        if (raw === undefined || raw === '') {
            ...
            if (allowed && allowed.size === 0)
                return { live: false, test: false, testLabel: null, testScope: null };
    ...
    app.get('/api/v1/pay/:id', async (req, reply) => {
        ...
        if (depositOpen && choice !== 'deposit' && choice !== 'full') {
            ...
            return reply.type('text/html').header('cache-control', 'no-store').header('referrer-policy', 'no-referrer').send(`<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">`
                + `<meta name="robots" content="noindex, nofollow">`
