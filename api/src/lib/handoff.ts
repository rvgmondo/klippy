...
export interface DealWonPayload extends Record<string, unknown> {
  dealId: number;
...
on<DealWonPayload>('deal.won', 'create-client-workspace', async (p, ctx): Promise<HandlerResult> => {
  ...
  if (!deal) return { outcome: 'deal no longer exists', ok: false };
  ...
  const folderId = await db.transaction(async (tx) => {
    ...
    const fIns = await tx.insert(folders).values(withTenant(accountId, {
      parentId: null, businessId: deal.businessId, name, pillar: 'delivery' as const,
...
on<DealWonPayload>('deal.won', 'draft-opening-invoice', async (p, ctx): Promise<HandlerResult> => {
  const { accountId, userId } = ctx;
  if (!p.value || p.value <= 0) return { outcome: 'no value on the deal, nothing to invoice', ok: true };
  ...
  if (folderId) {
    const [existing] = await db.select({ id: documents.id, number: documents.number }).from(documents)
      .where(tenantWhere(documents, accountId,
        eq(documents.type, 'invoice'), eq(documents.folderId, folderId))).limit(1);
    if (existing) return { outcome: `invoice ${existing.number} already exists`, ok: true };
  }
  ...
  const docId = await db.transaction(async (tx) => {
    const ins = await tx.insert(documents).values(withTenant(accountId, {
      type: 'invoice' as const, seq, number, businessId: p.businessId, folderId,
...
on<DealWonPayload>('deal.won', 'notify-team', async (p, ctx): Promise<HandlerResult> => {
  const { accountId, userId } = ctx;
  ...
  if (!targets.length) return { outcome: 'nobody else to tell', ok: true };
