  ...
  for (const sub of due) {
    ...
    const claim = await db.update(subscriptions)
      ...
      .where(and(
        ...
        eq(subscriptions.nextBillDate, sub.nextBillDate),
        eq(subscriptions.status, 'active'),
      ));
    if (!claim[0].affectedRows) { skipped++; continue; }
    try {
      ...
      try {
        ...
        }).catch(() => { /* never fail a billing run over a log write */ });
      }
 ...
 */
export async function runBackupEmail(): Promise<string> {
  // getUTCDay: 0 is Sunday. Other days record a clean skip.
  if (new Date().getUTCDay() !== 0) return 'Not Sunday; nothing sent.';
  ...
  const owners = await db.select({
    ...
    .where(and(eq(memberships.role, 'owner'), eq(memberships.isActive, true),
      eq(users.isActive, true), eq(accounts.status, 'active')));

  const byAccount = new Map<number, typeof owners>();
  ...
  for (const [accountId, list] of byAccount) {
    try {
      ...
      for (const o of list) {
        await sendMail(o.email, `Your weekly Klippy backup (${o.accountName})`,
          renderEmailText(bkBrand, bkContent), renderEmail(bkBrand, bkContent), [attachment]);
        sent++;
      }
    } catch {
 ...
 */
async function claimJobForToday(name: string, today: string): Promise<boolean> {
...
export async function runDueJobs(): Promise<void> {
  ...
  for (const job of JOBS) {
    ...
    if (s?.lastRunOn === today) continue;   // cheap fast path; the claim is the real gate
    if (hour < job.hour) continue;
    // Only the caller that wins the atomic claim runs the job.
    if (!(await claimJobForToday(job.name, today))) continue;
