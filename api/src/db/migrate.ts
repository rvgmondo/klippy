...
function checkConfig(): void {
  ...
  const off: string[] = [];
  if (!process.env.PAYMENTS_SECRET) off.push('online payments + saved cards (PAYMENTS_SECRET unset)');
  if (!process.env.SMTP_HOST) off.push('outbound email: invoices, reminders, receipts, portal invites (SMTP_HOST unset)');
  if (!process.env.APP_URL) off.push('correct links in emails (APP_URL unset)');
  if (!process.env.VAPID_PUBLIC_KEY || !process.env.VAPID_PRIVATE_KEY) off.push('web push notifications (VAPID keys unset)');
  if (!process.env.CORS_ORIGIN) off.push('cross-origin browser access (CORS_ORIGIN unset; requests will be denied)');
  if (!process.env.JWT_SECRET) off.push('a STABLE session secret (JWT_SECRET unset; sessions drop on restart)');
  if (off.length) {
