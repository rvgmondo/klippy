...
export const PRIMITIVES = ['acquisition', 'fulfillment', 'finance', 'admin'] as const;
...
export interface ModuleDef {
  ...
  label: string;
  primitive: Primitive;
  /** Never hidden: without these Klippy stops being usable. */
  core?: boolean;
  /** Business types this is on for by default. Omitted means every type. */
...
export function defaultModulesFor(type: BusinessType, secondary: BusinessType[] = []): string[] {
...
export function effectiveModules(
  stored: string[] | null | undefined, type: BusinessType, secondary: BusinessType[] = [],
): string[] {
  const chosen = stored?.length ? stored.filter((k) => BY_KEY.has(k)) : defaultModulesFor(type, secondary);
  const withCore = new Set(chosen);
  for (const m of MODULES) if (m.core) withCore.add(m.key);
