...
import { SocialApiError, dryRun, } from '../types.js';
/**
 ...
 */
export async function metaListAccounts(userToken) {
    const pages = await httpJson(`${GRAPH}/me/accounts?${new URLSearchParams({
...
async function containerStatus(id, token) {
    return httpJson(`${GRAPH}/${id}?${new URLSearchParams({ fields: 'status_code,status', access_token: token })}`, { network: 'instagram' });
}
/**
...
async function waitForContainer(id, token) {
    ...
    while (Date.now() < deadline) {
        ...
        if (s.status_code === 'ERROR') {
            throw new SocialApiError('instagram', `Instagram could not process the media: ${s.status ?? 'no reason given'}`, { retryable: false, code: 'container-error', detail: s });
        }
        if (s.status_code === 'EXPIRED') {
            throw new SocialApiError('instagram', 'The upload expired before it was published.', { retryable: true, code: 'container-expired', detail: s });
        }
    ...
    throw new SocialApiError('instagram', 'Instagram is still processing the video after five minutes.', { retryable: true, code: 'container-timeout' });
}
async function createContainer(igUserId, token, params) {
    const form = { access_token: token };
    for (const [k, v] of Object.entries(params))
        if (v !== undefined)
            form[k] = v;
    const res = await httpJson(`${GRAPH}/${igUserId}/media`, {
        method: 'POST', form, network: 'instagram',
    });
    if (!res?.id) {
        throw new SocialApiError('instagram', 'Instagram accepted the upload but returned no id.', { retryable: true, code: 'no-container-id' });
    }
    return res.id;
}
async function publishInstagram(creds, post, media) {
    ...
    if (!usable.length) {
        ...
        throw new SocialApiError('instagram', 'Instagram fetches media from a public URL and none of these files have one it could reach. Check PUBLIC_MEDIA_BASE.', { retryable: false, code: 'no-public-media' });
    }
    let containerId;
    ...
    // IG-PUB-07.
    const published = await httpJson(`${GRAPH}/${igId}/media_publish`, {
    ...
    let permalink = null;
    try {
        const info = await httpJson(`${GRAPH}/${published.id}?${new URLSearchParams({ fields: 'permalink', access_token: token })}`, { network: 'instagram' });
...
// ---- Facebook Page -------------------------------------------------------------
async function publishFacebook(creds, post, media) {
    const token = creds.accessToken;
    ...
    if (videos.length) {
        const res = await httpJson(`${GRAPH}/${pageId}/videos`, {
            method: 'POST',
    ...
    if (photos.length === 1) {
        const res = await httpJson(`${GRAPH}/${pageId}/photos`, {
            method: 'POST',
            form: {
    ...
    if (!message) {
        throw new SocialApiError('facebook', 'A Facebook post needs either text or a photo.', { retryable: false, code: 'empty' });
    }
    const res = await httpJson(`${GRAPH}/${pageId}/feed`, {
...
function buildAdapter(network) {
    return {
        ...
        async publish(creds, post, media) {
            if (dryRun()) {
 ...
 */
export async function instagramQuota(igUserId, token) {
    try {
        const res = await httpJson(`${GRAPH}/${igUserId}/content_publishing_limit?${new URLSearchParams({
