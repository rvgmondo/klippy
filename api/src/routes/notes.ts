...
export async function noteRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/notes', async (req, reply) => {
    ...
    if (role === 'member') return reply.code(403).send({ error: 'Admins only.' });
  ...
  app.patch('/api/v1/notes/:id', async (req, reply) => {
    ...
    if (!res[0].affectedRows) return reply.code(404).send({ error: 'Note not found.' });
  ...
  app.get('/api/v1/notes/export', async (req, reply) => {
    ...
    const lines: string[] = ['# Klippy product notes', ''];
    if (!notes.length) lines.push('_Nothing outstanding._');

    for (const group of ['bug', 'improvement', 'idea', 'question'] as const) {
