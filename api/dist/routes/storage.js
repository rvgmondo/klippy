...
function childrenWhere(accountId, parentId) {
...
async function wouldCycle(accountId, nodeId, candidateParent) {
 ...
 */
async function collectSubtree(accountId, rootId) {
    const out = [];
    const queue = [rootId];
    while (queue.length) {
...
export async function storageRoutes(app) {
    ...
    app.get('/api/v1/storage', async (req, reply) => {
        ...
        if (!q.success)
            return reply.code(400).send({ error: 'Bad parentId.' });
    ...
    app.post('/api/v1/storage/folder', async (req, reply) => {
        ...
        const ins = await db.insert(storageNodes).values(withTenant(accountId, {
            parentId, kind: 'folder', name: parsed.data.name,
        }));
    ...
    app.post('/api/v1/storage/upload', async (req, reply) => {
        ...
        const ins = await db.insert(storageNodes).values(withTenant(accountId, {
            parentId, kind: 'file', name: part.filename.slice(0, 255),
    ...
    app.patch('/api/v1/storage/:id', async (req, reply) => {
        ...
        if (parsed.data.parentId !== undefined && parsed.data.parentId !== null) {
            ...
            if (!parent || parent.kind !== 'folder')
                return reply.code(400).send({ error: 'Target folder not found.' });
    ...
    app.delete('/api/v1/storage/:id', async (req, reply) => {
        ...
        for (const n of subtree)
            if (n.storageKey)
                await storage().delete(n.storageKey);
    ...
    app.get('/api/v1/storage/:id/download', async (req, reply) => {
        ...
        const [node] = await db.select().from(storageNodes)
            .where(tenantWhere(storageNodes, accountId, eq(storageNodes.id, id))).limit(1);
        if (!node || node.kind !== 'file' || !node.storageKey)
            return reply.code(404).send({ error: 'File not found.' });
        if (!(await storage().exists(node.storageKey)))
        ...
        reply.header('Content-Disposition', `attachment; filename="${node.name.replace(/[^\x20-\x7e]/g, '_').replace(/"/g, "'")}"`);
