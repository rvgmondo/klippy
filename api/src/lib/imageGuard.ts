...
export interface ImageCheck { ok: boolean; reason?: string; width?: number; height?: number; kind?: 'png' | 'jpeg' }

const PNG_SIG = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
/** Well beyond any sane logo, but low enough that decoding cannot run away. */
const MAX_DIMENSION = 10000;

function checkPng(buf: Buffer): ImageCheck {
  if (buf.length < 57) return { ok: false, reason: 'The file is too small to be a real PNG.' };
  if (!buf.subarray(0, 8).equals(PNG_SIG)) return { ok: false, reason: 'Not a PNG.' };
  if (buf.toString('ascii', 12, 16) !== 'IHDR') return { ok: false, reason: 'The PNG header is missing.' };

  const width = buf.readUInt32BE(16);
  const height = buf.readUInt32BE(20);
  if (!width || !height) return { ok: false, reason: 'The image reports no size.' };
  ...
  while (off + 12 <= buf.length) {
    ...
    if (!/^[a-zA-Z]{4}$/.test(type)) return { ok: false, reason: 'The PNG is damaged.' };
  ...
  if (!sawEnd) return { ok: false, reason: 'The PNG is incomplete. Try re-exporting it.' };
  return { ok: true, width, height, kind: 'png' };
}

function checkJpeg(buf: Buffer): ImageCheck {
  if (buf.length < 4) return { ok: false, reason: 'The file is too small to be a real JPEG.' };
  if (buf[0] !== 0xff || buf[1] !== 0xd8) return { ok: false, reason: 'Not a JPEG.' };

  // Walk the markers to the frame header, which carries the dimensions.
  let off = 2;
  while (off + 4 <= buf.length) {
    if (buf[off] !== 0xff) return { ok: false, reason: 'The JPEG is damaged.' };
 ...
 */
export function checkImage(buf: Buffer): ImageCheck {
  if (!buf || buf.length < 8) return { ok: false, reason: 'The file is empty.' };
  if (buf.subarray(0, 8).equals(PNG_SIG)) return checkPng(buf);
...
export function safeImage(buf: Buffer | null): Buffer | null {
  if (!buf) return null;
