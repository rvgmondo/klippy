 ...
 */
const OPEN_DEAL_STAGES = ['lead', 'contacted', 'proposal'];
...
export async function focusRoutes(app) {
    ...
    app.get('/api/v1/focus', async (req) => {
        ...
        const importance = (kind, refId) => judged.get(kind + ':' + refId) ?? true;
        ...
        const quadrantOf = (i) => i.urgent ? (i.important ? 'now' : 'quick') : (i.important ? 'real' : 'later');
        const quadrants = { now: [], real: [], quick: [], later: [] };
        for (const i of items)
            quadrants[quadrantOf(i)].push(i);
    ...
    app.patch('/api/v1/focus/:id', async (req, reply) => {
        ...
        if (parsed.data.important !== undefined)
            patch.important = parsed.data.important;
