...
/**
 * The daily command centre, and the bottleneck.
 *
 * A dashboard that lists everything tells you nothing. Home used to show counts and
 * an "Up next" box that read "Nothing due. Clear runway." while an invoice sat
 * three weeks overdue, because the box only ever looked at task due dates. So this
 * asks a different question: across every part of the business, what are the few
 * things that most need me today, and which single constraint is holding the whole
 * thing back?
 *
 * The constraint idea is borrowed from the theory of constraints: a business has
 * one binding limit at a time, and work spent anywhere else does not move the
 * output. Four candidates are scored and only the worst is named, because "here is
 * your problem" is useful and "here are four problems" is another list.
 */
...
interface FeedItem {
  ...
  title: string;
  detail: string;
  /** Where in the app to go and deal with it. */
  view: string;
  amount?: number;
  days?: number;
...
export async function commandRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/command-centre', async (req) => {
    ...
    const bizFilter = (col: Parameters<typeof eq>[0]) =>
      onlyBusiness ? eq(col as never, onlyBusiness) : undefined;
    ...
    if (invIds.length) {
      const pays = await db.select({ documentId: payments.documentId, amount: sql<string>`SUM(${payments.amount})` })
        .from(payments).where(tenantWhere(payments, accountId, inArray(payments.documentId, invIds)))
        .groupBy(payments.documentId);
      for (const p of pays) paidBy.set(p.documentId, Number(p.amount));
      const creds = await db.select({ src: documents.sourceDocumentId, amount: sql<string>`SUM(${documents.total})` })
        .from(documents).where(tenantWhere(documents, accountId,
          eq(documents.type, 'credit_note'), inArray(documents.sourceDocumentId, invIds), ne(documents.status, 'void')))
        .groupBy(documents.sourceDocumentId);
      for (const c of creds) if (c.src != null) creditedBy.set(c.src, Number(c.amount));
    }
    ...
    const dueSubs = await db.select({
      ...
      .where(tenantWhere(subscriptions, accountId,
        eq(subscriptions.status, 'active'), lte(subscriptions.nextBillDate, today),
        bizFilter(subscriptions.businessId),
    ...
    const rank: Record<Urgency, number> = { critical: 0, high: 1, normal: 2 };
