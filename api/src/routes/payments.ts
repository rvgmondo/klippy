 ...
 */
function clientPage(reply: FastifyReply, p: {
  title: string; brand?: string | null; lines: string[]; link?: { href: string; label: string };
}) {
...
async function clientFacingBrand(accountId: number, businessId: number | null): Promise<string | null> {
 ...
 */
function returnUrlsFor(docId: number): { returnUrl: string; cancelUrl: string } | null {
...
export async function paymentRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/documents/:id/pay-link', { preHandler: app.requireAuth }, async (req, reply) => {
    ...
    if (owed <= 0.001) return reply.code(400).send({ error: 'Nothing is outstanding on this invoice.' });
  ...
  app.get('/api/v1/pay/:id', async (req, reply) => {
    ...
    const page = (title: string, msg: string) => clientPage(reply, { title, lines: [msg] });

    const id = Number((req.params as { id: string }).id);
    const token = (req.query as { t?: string }).t ?? '';
    if (!id || !verifyPayToken(id, token)) return page('Link not valid', 'This payment link is invalid or has expired.');
    ...
    return reply.type('text/html').header('cache-control', 'no-store').send(
      `<!doctype html><meta charset="utf-8"><meta name="color-scheme" content="light"><title>Redirecting to PayFast</title>`
      + `<body style="margin:0;background:#f8fafc">`
  ...
  app.get('/api/v1/pay/:id/return', async (req, reply) => {
    const id = Number((req.params as { id: string }).id);
    const q = req.query as { t?: string; r?: string };
    const token = q.t ?? '';
  ...
  app.post('/api/v1/payfast/notify', async (req, reply) => {
    const payload = req.body as { raw: string; parsed: Record<string, string> } | undefined;
    ...
    try {
       ...
       * the problem.
       */
      const record = async (outcome: string, ok2: boolean) => {
        ...
        try {
          ...
          const content = {
            ...
            facts: [
              ...
              ['Paid', formatMoney(Number(body.amount_gross), doc.currency)] as [string, string],
              ['Reference', pfId || 'PayFast'] as [string, string],
            ],
          };
