...
export async function reportRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/reports/time', async (req, reply) => {
    ...
    for (const m of mrrRows) into(m.currency).mrr += m.mrr;
  ...
  app.get('/api/v1/reports/profit', async (req, reply) => {
    ...
    const curOf = (bid: number | null) => (bid != null ? bizById.get(bid)?.currency : null) || wsCur;
    ...
    const invRows = await db.select({
      ...
      .where(tenantWhere(documents, accountId, scope, onlyDoc,
        inArray(documents.type, ['invoice', 'credit_note']),
    ...
    const saleRows = await db.select({
      ...
      .where(tenantWhere(sales, accountId,
        ...
        only !== undefined ? eq(sales.businessId, only) : undefined,
        gte(sales.occurredAt, fromDate), lte(sales.occurredAt, toDate)));

    const expRows = await db.select({ businessId: expenses.businessId, amount: expenses.amount })
      .from(expenses)
      .where(tenantWhere(expenses, accountId,
        await businessScope(req, expenses.businessId),
        only !== undefined ? eq(expenses.businessId, only) : undefined,
        gte(expenses.incurredOn, from), lte(expenses.incurredOn, to)));
    ...
    const payRows = await db.select({
      ...
      .where(tenantWhere(payments, accountId, scope, onlyDoc,
        gte(payments.paidOn, from), lte(payments.paidOn, to)));

    type Row = {
      businessId: number | null; name: string; currency: string;
      invoiced: number; taken: number; received: number;
      expenses: number; fees: number;
    };
    const rows = new Map<string, Row>();
    const into = (businessId: number | null, currency: string): Row => {
    ...
    for (const p of payRows) {
      ...
      if (p.fee != null) r.fees += Number(p.fee);
    }
  ...
  app.get('/api/v1/reports/vat', async (req, reply) => {
    ...
    const { from, to } = q.data;
    const onlyBusiness = q.data.businessId;
    const pick = <T extends { businessId: number | null }>(rows: T[]) =>
      onlyBusiness === undefined ? rows : rows.filter((r) => r.businessId === onlyBusiness);
    ...
    const salesRows = await db.select({ currency: sales.currency, tax: sales.taxAmount, gross: sales.gross, businessId: sales.businessId })
      ...
      .where(tenantWhere(sales, accountId,
        await businessScope(req, sales.businessId),
        gte(sales.occurredAt, new Date(`${from}T00:00:00.000Z`)),
        lte(sales.occurredAt, new Date(`${to}T23:59:59.999Z`))));
    ...
    const intoCur = (c: string) => {
      let b = byCur.get(c);
      if (!b) { b = { outputVat: 0, sales: 0 }; byCur.set(c, b); }
      return b;
    };
    // Per entity: the figures somebody would actually put on a return.
    type Entity = { outputVat: number; sales: number; inputVat: number; currency: string };
    const byBiz = new Map<number | null, Entity>();
    const intoBiz = (bid: number | null) => {
      let b = byBiz.get(bid);
      if (!b) {
    ...
    for (const r of saleRows) {
      ...
      c.outputVat += Number(r.tax); c.sales += Number(r.gross);
      const e = intoBiz(r.businessId);
      e.outputVat += Number(r.tax); e.sales += Number(r.gross);
    }
    for (const r of invRows) {
      const sign = r.type === 'credit_note' ? -1 : 1;
      const c = intoCur(r.currency);
      c.outputVat += sign * Number(r.tax); c.sales += sign * Number(r.total);
      const e = intoBiz(r.businessId);
      e.outputVat += sign * Number(r.tax); e.sales += sign * Number(r.total);
    }
    ...
    for (const e of expRows) intoBiz(e.businessId).inputVat += Number(e.vat ?? 0);
  ...
  app.get('/api/v1/reports/unbilled', async (req, reply) => {
    ...
    const bizRows = await db.select({ id: businesses.id, currency: businesses.currency })
      .from(businesses).where(tenantWhere(businesses, accountId));
    const curOf = (bid: number | null) =>
      (bid != null ? bizRows.find((b) => b.id === bid)?.currency : null) || workspaceCurrency;
    ...
    const perClient = new Map<number, { name: string; currency: string; rate: number | null;
      budget: number | null; unbilledSeconds: number; monthSeconds: number }>();
    for (const e of entries) {
      const root = rootOf(e.folderId);
      ...
      const secs = Number(e.seconds ?? 0);
      if (e.billed == null) c.unbilledSeconds += secs;
      if (e.startTime >= monthStart) c.monthSeconds += secs;
    }
    // Budgeted clients with no time at all this month still belong on the list.
    for (const f of allFolders) {
      if (f.parentId != null || f.monthlyHoursBudget == null || f.deletedAt) continue;
      if (!canBiz(f.businessId ?? null)) continue;
      if (onlyBusiness !== undefined && f.businessId !== onlyBusiness) continue;
