...
export function sampleNames() {
    ...
    for (const t of Object.values(TEMPLATES)) {
        ...
        for (const area of [...t.delivery, ...t.operations])
            foldersList.push(area.name);
        ...
        for (const dl of t.deals) {
            if (dl.company)
                companies.push(dl.company);
            dealTitles.push(dl.title);
        }
        for (const of2 of t.offerings)
            offeringsList.push(of2.name);
    }
    return {
