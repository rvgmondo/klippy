...
import { PORTAL_COOKIE, LINK_TTL_MINUTES, consumeLoginToken, issueLoginTokensForEmail, issueLoginTokenForUser, passwordLogin, portalContext, portalCookieOptions, setPortalPassword, clearPortalPassword, signPortalToken, signPreviewToken, normaliseEmail, generatePortalPassword, } from '../lib/portalAuth.js';
...
export async function portalRoutes(app) {
    ...
    app.get('/api/v1/portal/report', async (req, reply) => {
        ...
        if (!q.success)
            return reply.code(400).send({ error: 'Bad month.' });
        ...
        const subtree = new Set([c.user.folderId]);
        let grew = true;
        while (grew) {
            grew = false;
        ...
        const boardRows = await db.select({ id: boards.id, name: boards.name }).from(boards)
            .where(and(eq(boards.accountId, c.user.accountId), inArray(boards.folderId, [...subtree]), isNull(boards.deletedAt)));
        if (!boardRows.length)
            return { month, boards: [], completed: [], totalHours: 0 };
        ...
        const time = await db.select({
            ...
            .where(and(eq(timeEntries.accountId, c.user.accountId), inArray(tasks.boardId, boardIds), gte(timeEntries.startTime, start), lt(timeEntries.startTime, end)))
        ...
        const done = await db.select({
            ...
            .where(and(eq(tasks.accountId, c.user.accountId), inArray(tasks.boardId, boardIds), eq(tasks.isCompleted, true), gte(tasks.completedAt, start), lt(tasks.completedAt, end)))
            .orderBy(desc(tasks.completedAt))
            .limit(100);
        const hoursOf = (secs) => Math.round((secs / 3600) * 10) / 10;
    ...
    app.get('/api/v1/portal/hosting', async (req, reply) => {
        ...
        const subIds = rows.map((r) => r.subscriptionId).filter((n) => n != null);
    ...
    app.post('/api/v1/portal/hosting/:id/domain', async (req, reply) => {
        ...
        const [existing] = await db.select({ isTemporary: hostingAccounts.isTemporary })
            .from(hostingAccounts)
            .where(and(eq(hostingAccounts.accountId, c.user.accountId), eq(hostingAccounts.subscriptionId, subscriptionId))).limit(1);
    ...
    app.get('/api/v1/portal/hosting/awaiting', async (req, reply) => {
        ...
        const accounts = ids.length
            ? await db.select({
                ...
                .where(and(eq(hostingAccounts.accountId, c.user.accountId), inArray(hostingAccounts.subscriptionId, ids)))
            : [];
...
export async function portalAdminRoutes(app) {
    ...
    app.post('/api/v1/folders/:id/portal-users', async (req, reply) => {
        ...
        if (parsed.data.invite !== false)
            await sendInvite(newPortalUserId);
    ...
    app.post('/api/v1/portal-users/:id/password', async (req, reply) => {
        const { accountId, userId, role } = authOf(req);
        if (role === 'member')
            return reply.code(403).send({ error: 'Only admins can change portal access.' });
        const id = intId(req);
        if (!id)
            return reply.code(400).send({ error: 'Bad id.' });
        const parsed = z.object({
        ...
        if (parsed.data.sendLink && u.isActive) {
            await sendInvite(u.id).then(() => { linkSent = true; }).catch(() => { });
        }
 ...
 */
async function sendInvite(portalUserId) {
