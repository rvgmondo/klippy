 ...
 */
export function sanitiseTemplate(input) {
    if (!input)
        return '';
    let html = input;
    ...
    html = html.replace(/<\/?([a-zA-Z0-9-]+)([^>]*)>/g, (_m, rawName, attrs) => {
 ...
 */
export function htmlToPdfRuns(html) {
    if (!html)
        return [];
    const runs = [];
    let bold = false, italic = false, heading = 0, bullet = false;
    let buf = '';
    const flush = () => {
        const text = decode(buf).replace(/[ \t]+/g, ' ').trim();
        if (text)
            runs.push({ text, bold, italic, heading, bullet });
        buf = '';
    };
    ...
    for (const t of tokens) {
        ...
        switch (name) {
            ...
            case 'h3':
                flush();
                heading = closing ? 0 : Number(name[1]);
                break;
            case 'li':
                flush();
                bullet = !closing;
...
export function templateDataFor(doc, business, account) {
    ...
    return {
        ...
        'client.vat': doc.clientVatNumber ?? '',
        'invoice.number': doc.number ?? '',
        'invoice.issueDate': doc.issueDate ?? '',
