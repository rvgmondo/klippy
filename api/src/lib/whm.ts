...
export function isReservedUsername(u: string): boolean {
  if (RESERVED_NAMES.has(u)) return true;
...
export function usernameFor(domain: string, taken: (u: string) => boolean = () => false): string {
  ...
  if (isReservedUsername(stem)) stem = `k${stem}`.slice(0, 12);

  let candidate = stem;
  let n = 0;
  while (taken(candidate) || isReservedUsername(candidate)) {
    n += 1;
    if (n > 50) return `k${Math.random().toString(36).slice(2, 10)}`;
 ...
 */
export function tempDomainFor(pattern: string, username: string): string | null {
  ...
  if (!/^[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)+$/.test(filled)) return null;
  if (filled.length > 190) return null;
  if (!pattern.includes('{username}')) return null;
  return filled;
}

/** Did WHM refuse this name rather than fail for some other reason? */
export function isReservedRejection(message: string): boolean {
...
export async function changePrimaryDomain(
  creds: WhmCreds, username: string, newDomain: string,
): Promise<WhmResult> {
 ...
 */
export async function setAccountPassword(creds: WhmCreds, username: string, password: string): Promise<WhmResult> {
