...
export async function runFinanceDigest() {
    ...
    for (const [accountId, people] of byAccount) {
        ...
        const into = (c) => { let b = bucket.get(c); if (!b) {
            b = { invoiced: 0, received: 0, owed: 0, fees: 0, taken: 0 };
            bucket.set(c, b);
        } return b; };
        ...
        for (const r of payRows) {
            ...
            if (r.fee != null)
                into(r.currency).fees += Number(r.fee);
        }
        ...
        for (const r of rows) {
            ...
            if (r.taken > 0)
                facts.push([`Taken over the counter (${r.currency})`, formatMoney(r.taken, r.currency)]);
            if (r.fees > 0) {
