...
export async function leadRoutes(app) {
    ...
    app.get('/api/v1/businesses/:id/lead-link', { preHandler: app.requireAuth }, async (req, reply) => {
        ...
        if (!(await assertBusinessAccess(req, reply, id, 'member')))
            return;
    ...
    app.post('/api/v1/public/lead/:id', {
        ...
        const id = Number(req.params.id);
        const t = req.query.t ?? '';
        const fromForm = req.query.form === '1';
        if (!id || !verifyLeadToken(id, t))
            return reply.code(404).send({ error: 'Not found.' });
        ...
        if (!biz)
            return reply.code(404).send({ error: 'Not found.' });
        const [posRow] = await db.select({ pos: sql `COALESCE(MAX(position), 0) + 1` })
