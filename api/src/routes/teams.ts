...
export async function teamRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/teams', async (req, reply) => {
    ...
    if (role === 'member') return reply.code(403).send({ error: 'Only admins can create teams.' });
  ...
  app.patch('/api/v1/teams/:id', async (req, reply) => {
    const { accountId, role } = authOf(req);
    if (role === 'member') return reply.code(403).send({ error: 'Only admins can edit teams.' });
    const id = intId(req);
    ...
    if (!res[0].affectedRows) return reply.code(404).send({ error: 'Team not found.' });
  ...
  app.delete('/api/v1/teams/:id', async (req, reply) => {
    const { accountId, role } = authOf(req);
    if (role === 'member') return reply.code(403).send({ error: 'Only admins can delete teams.' });
    const id = intId(req);
    if (!id) return reply.code(400).send({ error: 'Bad id.' });
  ...
  app.post('/api/v1/teams/:id/members', async (req, reply) => {
    const { accountId, role } = authOf(req);
    if (role === 'member') return reply.code(403).send({ error: 'Only admins can change teams.' });
    const id = intId(req);
    if (!id) return reply.code(400).send({ error: 'Bad id.' });
  ...
  app.post('/api/v1/boards/:id/teams', async (req, reply) => {
    const { accountId, role } = authOf(req);
    if (role === 'member') return reply.code(403).send({ error: 'Only admins can change board teams.' });
    const id = intId(req);
    if (!id) return reply.code(400).send({ error: 'Bad id.' });
    const parsed = z.object({ teamId: z.number().int().positive() }).safeParse(req.body);
    if (!parsed.success) return reply.code(400).send({ error: 'teamId required.' });
  ...
  app.delete('/api/v1/boards/:id/teams/:teamId', async (req, reply) => {
    ...
    if (!id || !teamId) return reply.code(400).send({ error: 'Bad id.' });
