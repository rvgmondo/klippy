...
import { SALES_PROVIDERS, providerFor } from '../lib/salesProviders.js';
...
export async function salesRoutes(app) {
    ...
    app.get('/api/v1/sales/connections', async (req) => {
        ...
        const rows = await db.select({
            ...
            hasKey: sql `${paymentConnections.secretEnc} is not null`,
            businessName: businesses.name,
        }).from(paymentConnections)
            .leftJoin(businesses, eq(businesses.id, paymentConnections.businessId))
            .where(tenantWhere(paymentConnections, accountId, await businessScope(req, paymentConnections.businessId)));
        return {
    ...
    app.put('/api/v1/businesses/:id/sales-connection', async (req, reply) => {
        ...
        if (!(await assertBusinessAccess(req, reply, businessId, 'admin')))
            return;
        ...
        if (!provider)
            return reply.code(400).send({ error: 'That is not a payment provider Klippy knows about.' });
        if (!provider.client) {
        ...
        const scope = tenantWhere(paymentConnections, accountId, eq(paymentConnections.businessId, businessId), eq(paymentConnections.provider, provider.key));
        ...
        if (d.secret) {
            ...
            if (!trial.ok)
                return reply.code(400).send({ error: trial.message });
        }
        ...
        const patch = {};
        if (d.secret)
            patch.secretEnc = encryptSecret(d.secret);
        if (d.label !== undefined)
            patch.label = d.label;
        if (d.enabled !== undefined)
            patch.enabled = d.enabled;
        if (existing)
            await db.update(paymentConnections).set(patch).where(scope);
        else {
    ...
    app.delete('/api/v1/businesses/:id/sales-connection', async (req, reply) => {
        ...
        const key = (which.success && which.data.provider ? which.data.provider : 'yoco');
        ...
        await db.delete(paymentConnections).where(tenantWhere(paymentConnections, accountId, eq(paymentConnections.businessId, businessId), eq(paymentConnections.provider, key)));
    ...
    app.post('/api/v1/businesses/:id/sales-connection/sync', async (req, reply) => {
        ...
        const key = (body.success && body.data.provider ? body.data.provider : 'yoco');
        const res = await syncProvider(accountId, businessId, key, { fullResync: body.success && body.data.fullResync });
        if (!res.ok)
    ...
    app.get('/api/v1/sales', async (req, reply) => {
        ...
        const rows = await db.select().from(sales)
            .where(tenantWhere(sales, accountId, await businessScope(req, sales.businessId), q.data.businessId ? eq(sales.businessId, q.data.businessId) : undefined, gte(sales.occurredAt, new Date(`${q.data.from}T00:00:00.000Z`)), lte(sales.occurredAt, new Date(`${q.data.to}T23:59:59.999Z`))))
    ...
    app.post('/api/v1/sales', async (req, reply) => {
        ...
        const ins = await db.insert(sales).values(withTenant(accountId, {
            businessId: d.businessId, provider: 'manual', externalId: null,
