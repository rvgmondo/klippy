...
export async function userRoutes(app) {
    ...
    app.get('/api/v1/invitations', async (req, reply) => {
        const { accountId, role } = authOf(req);
        if (role === 'member')
            return reply.code(403).send({ error: 'Only admins can see invitations.' });
        const rows = await db.select({
            ...
            .where(and(eq(invitations.accountId, accountId), isNull(invitations.acceptedAt), isNull(invitations.revokedAt)))
    ...
    app.delete('/api/v1/invitations/:id', async (req, reply) => {
        const { accountId, role } = authOf(req);
        if (role === 'member')
            return reply.code(403).send({ error: 'Only admins can withdraw invitations.' });
        const id = intId(req);
        if (!id)
            return reply.code(400).send({ error: 'Bad id.' });
        await db.update(invitations).set({ revokedAt: new Date() })
            .where(and(eq(invitations.id, id), eq(invitations.accountId, accountId), isNull(invitations.acceptedAt)));
        return { ok: true };
    });
}
/**
...
export async function invitationRoutes(app) {
    app.post('/api/v1/invitations/accept', async (req, reply) => {
        ...
        if (!parsed.success)
            return reply.code(400).send({ error: 'That invitation link is incomplete.' });
        let userId = null;
