...
import {
  ...
  socialAccounts, storageNodes, businesses, users,
} from '../../db/schema.js';
...
import {
  NETWORK_LABEL, SocialApiError,
  type AdapterCredentials, type MediaItem, type Network, type PublishablePost,
} from './types.js';
 ...
 */
async function publishAutomatically(post: typeof socialPosts.$inferSelect): Promise<'published' | 'manual' | 'failed'> {
    ...
    try {
      ...
      if (post.firstComment && adapter.publishFirstComment) {
        try {
          ...
          await log(post.accountId, post.id, target.id, 'warn',
            `Posted, but the first comment did not go up: ${err instanceof Error ? err.message : 'unknown error'}`);
        }
      }
      ...
      await log(post.accountId, post.id, target.id, 'error',
        giveUp
          ? `${NETWORK_LABEL[network]} refused it: ${message}`
          : `${NETWORK_LABEL[network]} failed, trying again in ${waitMinutes} minutes: ${message}`,
        err instanceof SocialApiError ? err.detail : undefined);
    }
  }
  ...
  if (anyManual) await sendManualPrompt(post, { becauseNoAdapter: true, keepStatus: true });
...
async function mediaFor(post: typeof socialPosts.$inferSelect): Promise<MediaItem[]> {
 ...
 */
async function tellSomebodyItFailed(post: typeof socialPosts.$inferSelect): Promise<void> {
  ...
  const failures = await db.select({
    ...
    .where(tenantWhere(socialPostTargets, post.accountId,
      eq(socialPostTargets.postId, post.id), eq(socialPostTargets.status, 'failed')));
  ...
  const content = {
    ...
    body: [
      ...
      '',
      ...failures.map((f) => `${NETWORK_LABEL[f.network as Network]}: ${f.error ?? 'no reason given'}`),
      '',
  ...
  await sendMail(owner.email, `Did not go out: ${post.title}`,
    renderEmailText(brand, content), renderEmail(brand, content))
    .catch(() => { /* it is already flagged in the app */ });
}
...
async function sendManualPrompt(
  ...
  opts: { becauseNoAdapter?: boolean; keepStatus?: boolean } = {},
): Promise<void> {
