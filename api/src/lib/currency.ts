...
export interface Currency {
  code: string;
  name: string;
  /** For display where space is tight. Falls back to the code when ambiguous. */
  symbol: string;
  /** Minor units. 0 for yen, 3 for dinars, 2 for nearly everything else. */
  decimals: number;
}
...
export function currencyInfo(code: string | null | undefined): Currency {
 ...
 */
export function roundMoney(n: number, code: string | null | undefined): number {
 ...
 */
export function formatMoney(v: number | string, code: string | null | undefined): string {
 ...
 */
export function payfastSupports(code: string | null | undefined): boolean {
