...
export async function socialMediaRoutes(app: FastifyInstance) {
  app.get('/api/v1/m/:token', async (req, reply) => {
    ...
    // costs nothing.
    if (!/^[a-f0-9]{32,64}$/i.test(token)) return reply.code(404).send({ error: 'Not found.' });
    ...
    if (!row?.storageKey) return reply.code(404).send({ error: 'Not found.' });

    const type = row.mimeType ?? row.nodeMime ?? 'application/octet-stream';
    if (!SERVEABLE.has(type)) return reply.code(404).send({ error: 'Not found.' });
