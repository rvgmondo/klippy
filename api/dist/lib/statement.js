...
export async function buildStatement(accountId, folderId, opts = {}) {
    const { from, to } = opts;
    ...
    const allDocs = await db.select({
        ...
        .where(tenantWhere(documents, accountId, eq(documents.folderId, folderId), inArray(documents.type, ['invoice', 'credit_note']), ne(documents.status, 'void'), ne(documents.status, 'draft')));
    ...
    if (from) {
        for (const d of docs) {
            if (d.issueDate < from)
                opening += d.type === 'invoice' ? Number(d.total) : -Number(d.total);
        }
        for (const p of allPays) {
            if (p.paidOn < from)
                opening -= Number(p.amount);
        }
        opening = round(opening);
    }
    const inRange = (date) => (!from || date >= from) && (!to || date <= to);
    const raw = [
        ...docs.filter((d) => inRange(d.issueDate)).map((d) => d.type === 'invoice'
        ...
        ...allPays.filter((p) => inRange(p.paidOn)).map((p) => {
