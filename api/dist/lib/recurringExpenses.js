 ...
 */
export async function generateFor(accountId, row, today = todayStr()) {
    let written = 0;
    let skipped = 0;
...
export async function runRecurringExpenses() {
    ...
    const rows = await db.select().from(recurringExpenses)
        .where(and(eq(recurringExpenses.isActive, true), lte(recurringExpenses.nextDueOn, today), 
...
//# sourceMappingURL=recurringExpenses.js.map