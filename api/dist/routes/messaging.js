...
import { messagingFor, channelsFor, normalizePhone, phoneForClient, whatsappLink, reminderText, templateParams, sendSms, sendWhatsAppTemplate, } from '../lib/messaging.js';
/**
...
export async function messagingRoutes(app) {
    ...
    const write = async (accountId, businessId, body, reply) => {
        ...
        const patch = {};
        if (d.smsProvider !== undefined)
            patch.smsProvider = d.smsProvider;
        if (d.smsTokenId !== undefined)
            patch.smsTokenId = d.smsTokenId || null;
        if (d.smsTokenSecret !== undefined)
            patch.smsTokenSecretEnc = d.smsTokenSecret ? encryptSecret(d.smsTokenSecret) : null;
        if (d.smsSender !== undefined)
            patch.smsSender = d.smsSender || null;
        if (d.waPhoneNumberId !== undefined)
            patch.waPhoneNumberId = d.waPhoneNumberId || null;
        if (d.waAccessToken !== undefined)
            patch.waAccessTokenEnc = d.waAccessToken ? encryptSecret(d.waAccessToken) : null;
        if (d.waTemplateName !== undefined)
            patch.waTemplateName = d.waTemplateName || null;
        if (d.waTemplateLang !== undefined)
            patch.waTemplateLang = d.waTemplateLang || 'en';
        if (d.remindBySms !== undefined)
            patch.remindBySms = d.remindBySms;
        if (d.remindByWhatsapp !== undefined)
            patch.remindByWhatsapp = d.remindByWhatsapp;
    ...
    app.post('/api/v1/account/messaging/test', { preHandler: app.requireAuth }, async (req, reply) => {
        ...
        if (!parsed.success)
            return reply.code(400).send({ error: 'Enter a phone number and pick a channel.' });
        const to = normalizePhone(parsed.data.to);
        if (!to)
            return reply.code(400).send({ error: 'That does not look like a phone number.' });
        const row = await messagingFor(accountId, parsed.data.businessId || null);
        if (!row)
            return reply.code(400).send({ error: 'Save your messaging settings first.' });
        ...
        try {
            if (parsed.data.channel === 'sms')
                await sendSms(row, to, reminderText(facts));
            else
                await sendWhatsAppTemplate(row, to, templateParams(facts));
