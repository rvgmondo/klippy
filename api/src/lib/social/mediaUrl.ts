...
export function publicMediaUrl(token: string | null | undefined, mimeType: string | null | undefined): string | null {
 ...
 */
export function isPubliclyFetchable(url: string | null): boolean {
  if (!url) return false;
  try {
    const u = new URL(url);
