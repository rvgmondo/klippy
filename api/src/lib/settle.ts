...
import { balanceOf, type Balance } from './balances.js';
...
export async function settleIfCovered(
  accountId: number, docId: number, total: number, currentStatus: string,
): Promise<{ bal: Balance; settled: boolean; flipped: boolean }> {
  const bal = await balanceOf(accountId, docId, total);
  const settled = bal.outstanding <= 0.001;
  if (!settled || currentStatus === 'paid') return { bal, settled, flipped: false };

  const claim = await db.update(documents).set({ status: 'paid' })
    .where(and(
      tenantWhere(documents, accountId, eq(documents.id, docId)),
      ne(documents.status, 'paid'),
    ));
  const flipped = !!claim[0].affectedRows;
  if (flipped && bal.paid > 0.001) await onInvoicePaid(accountId, docId);
