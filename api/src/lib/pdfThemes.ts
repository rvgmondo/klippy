...
import { htmlToPdfRuns, type PdfRun } from './template.js';
...
export const PDF_TEMPLATES = ['modern', 'classic', 'bold', 'sidebar', 'compact'] as const;
export type PdfTemplate = (typeof PDF_TEMPLATES)[number];

export const PDF_TYPEFACES = ['sans', 'serif', 'mono'] as const;
export type PdfTypeface = (typeof PDF_TYPEFACES)[number];

export const TEMPLATE_INFO: { key: PdfTemplate; label: string; blurb: string }[] = [
...
export const TYPEFACE_INFO: { key: PdfTypeface; label: string; blurb: string }[] = [
 ...
 */
export const ISSUER_PLACEMENTS = ['footer', 'header', 'beside'] as const;
export type IssuerPlacement = (typeof ISSUER_PLACEMENTS)[number];

export const PLACEMENT_INFO: { key: IssuerPlacement; label: string; blurb: string }[] = [
...
interface FontSet { regular: string; bold: string; italic: string; boldItalic: string }
const FONTS: Record<PdfTypeface, FontSet> = {
...
export interface DocData {
  ...
  issueDate: string;
  dueDate: string | null;
  dueLabel: string;              // "Due" or "Valid until"
  paid: boolean;
  issuer: {
    name: string; address: string | null; vat: string | null; reg: string | null;
    bank: string | null; footer: string | null; logo: Buffer | null;
  };
...
export interface ThemeOptions {
  ...
  issuer?: IssuerPlacement;
}

type Doc = InstanceType<typeof PDFDocument>;

/** Everything a layout needs, so each one is only about arrangement. */
interface Ctx {
  pdf: Doc; d: DocData; f: FontSet; accent: string;
  W: number; H: number; M: number; right: number;
  issuerAt: IssuerPlacement;
}
...
function inkOn(accent: [number, number, number]): string {
  const [r, g, b] = accent;
 ...
 */
function drawLines(c: Ctx, y: number, opts: {
  size?: number; rowGap?: number; headerRule?: boolean; accentHeader?: boolean;
 ...
 */
function drawTotals(c: Ctx, y: number, opts: { boxed?: boolean; left?: number; width?: number }): number {
  const { pdf, d, f } = c;
  ...
  if (d.totals.discount) line('Discount', `-${d.totals.discount}`);
  if (d.totals.taxLabel && d.totals.tax) line(d.totals.taxLabel, d.totals.tax);
...
function drawRuns(c: Ctx, runs: PdfRun[], y: number, width: number, left = c.M): number {
  const { pdf, f } = c;
  for (const r of runs) {
    if (r.rule) {
...
function drawFooterBlocks(c: Ctx, y: number, width: number, left = c.M): number {
  const { pdf, d, f } = c;
  if (y > c.H - 130) { pdf.addPage(); y = c.M; }
...
function drawFinePrint(c: Ctx, left = c.M, width = c.right - c.M): void {
  const { pdf, d, f } = c;
  if (!d.issuer.footer) return;
...
function drawMeta(c: Ctx, x: number, y: number, width: number, align: 'left' | 'right' = 'right'): number {
  const { pdf, d, f } = c;
  const rows: [string, string][] = [
    ['Number', d.number],
    ['Issued', d.issueDate],
    ...(d.dueDate ? [[d.dueLabel, d.dueDate] as [string, string]] : []),
  ];
...
function drawBillTo(c: Ctx, x: number, y: number, width: number): number {
  const { pdf, d, f } = c;
  ...
  if (d.client.email) pdf.text(d.client.email, x, pdf.y + 1, { width });
  if (d.client.address) pdf.text(d.client.address, x, pdf.y + 1, { width });
  if (d.client.vat) pdf.text(`VAT ${d.client.vat}`, x, pdf.y + 1, { width });
  return pdf.y;
}

function issuerLines(d: DocData): string[] {
  return [
    d.issuer.address ?? '',
...
function issuerOneLine(d: DocData): string {
  return [
 ...
 */
function drawIssuerStrip(c: Ctx, left = c.M, width = c.right - c.M): void {
  const { pdf, d, f } = c;
  const details = issuerOneLine(d);
  const terms = d.issuer.footer;
  if (!details && !terms) return;
...
function layoutModern(c: Ctx): void {
  ...
  y = billEnd + 24;
  if (d.headerHtml) y = drawRuns(c, htmlToPdfRuns(d.headerHtml), y, c.right - c.M) + 14;
  y = drawLines(c, y, { accentHeader: true });
  y = drawTotals(c, y + 10, {});
  y = drawFooterBlocks(c, y + 12, c.right - c.M);
  if (d.footerHtml) drawRuns(c, htmlToPdfRuns(d.footerHtml), y + 6, c.right - c.M);

  // With the details at the foot the strip carries them AND the terms; otherwise
  // only the terms need a home down here.
  if (c.issuerAt === 'footer') drawIssuerStrip(c); else drawFinePrint(c);
}
...
function layoutClassic(c: Ctx): void {
  ...
  if (d.headerHtml) y = drawRuns(c, htmlToPdfRuns(d.headerHtml), y, c.right - c.M) + 12;
...
function layoutBold(c: Ctx): void {
  const { pdf, d, f } = c;
  const ink = inkOn([
    parseInt(c.accent.slice(1, 3), 16), parseInt(c.accent.slice(3, 5), 16), parseInt(c.accent.slice(5, 7), 16),
  ]);
  ...
  pdf.font(f.regular).fontSize(9).fillColor(ink)
    .text(`${d.issueDate}${d.dueDate ? `    ${d.dueLabel} ${d.dueDate}` : ''}`,
      c.right - 220, 104, { width: 220, align: 'right' });
...
function layoutSidebar(c: Ctx): void {
  const { pdf, d, f } = c;
  const SIDE = 168;
  ...
  y = billEnd + 20;
  if (d.headerHtml) y = drawRuns(c, htmlToPdfRuns(d.headerHtml), y, width, left) + 12;
  ...
  if (d.footerHtml) drawRuns(c, htmlToPdfRuns(d.footerHtml), y + 10, width, left);
  if (d.issuer.footer) {
    pdf.font(f.regular).fontSize(8).fillColor(MUTED)
...
function layoutCompact(c: Ctx): void {
  ...
  pdf.font(f.regular).fontSize(8).fillColor(MUTED)
    .text(`${d.issueDate}${d.dueDate ? `    ${d.dueLabel} ${d.dueDate}` : ''}`,
      c.right - 260, pdf.y + 2, { width: 260, align: 'right' });
  ...
  if (d.headerHtml) y = drawRuns(c, htmlToPdfRuns(d.headerHtml), y, c.right - c.M) + 10;
...
const LAYOUTS: Record<PdfTemplate, (c: Ctx) => void> = {
  modern: layoutModern,
...
export function drawDocument(pdf: Doc, d: DocData, o: ThemeOptions): void {
  ...
  const c: Ctx = {
    ...
    issuerAt: ISSUER_PLACEMENTS.includes(o.issuer as IssuerPlacement)
      ? (o.issuer as IssuerPlacement) : 'footer',
  };
  LAYOUTS[template](c);
}
