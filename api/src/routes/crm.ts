...
const ACTIVITY_KINDS = ['note', 'call', 'email', 'meeting', 'stage'] as const;
...
export async function crmRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/contacts', async (req, reply) => {
    ...
    const d = parsed.data;
    if (!(await assertMaybeBusiness(req, reply, d.businessId ?? null))) return;
  ...
  app.patch('/api/v1/contacts/:id', async (req, reply) => {
    ...
    if (!own) return reply.code(404).send({ error: 'Contact not found.' });
    if (!(await assertMaybeBusiness(req, reply, own.businessId))) return;

    const d = parsed.data;
    const patch: Record<string, unknown> = {};
    for (const k of ['name', 'phone', 'company', 'role', 'notes', 'folderId', 'businessId'] as const) {
