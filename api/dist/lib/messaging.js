 ...
 */
export function normalizePhone(raw, defaultCc = '27') {
    if (!raw)
        return null;
 ...
 */
export async function phoneForClient(accountId, folderId) {
    if (!folderId)
        return null;
...
export function reminderText(f) {
    return [
 ...
 */
export function templateParams(f) {
...
export async function messagingFor(accountId, businessId) {
    if (businessId) {
...
export function channelsFor(row) {
    if (!row)
...
export async function sendSms(row, to, body) {
 ...
 */
export async function sendWhatsAppTemplate(row, to, params) {
 ...
 */
export async function sendReminderMessage(accountId, businessId, to, facts, context) {
    const out = {};
    if (!to)
        return out;
    ...
    if (ch.sms) {
        try {
            ...
            out.sms = e instanceof Error ? e.message : 'failed';
        }
    }
    if (ch.whatsapp) {
        try {
            ...
            out.whatsapp = e instanceof Error ? e.message : 'failed';
        }
    }
