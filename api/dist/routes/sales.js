...
export async function salesRoutes(app) {
    ...
    app.put('/api/v1/businesses/:id/sales-connection', async (req, reply) => {
        ...
        const scope = tenantWhere(paymentConnections, accountId, eq(paymentConnections.businessId, businessId), eq(paymentConnections.provider, 'yoco'));
        ...
        if (existing)
            ...
            await db.insert(paymentConnections).values(withTenant(accountId, {
                businessId, provider: 'yoco', createdBy: userId, ...patch,
            }));
        }
        return { ok: true };
    });
    app.delete('/api/v1/businesses/:id/sales-connection', async (req, reply) => {
        ...
        await db.delete(paymentConnections).where(tenantWhere(paymentConnections, accountId, eq(paymentConnections.businessId, businessId), eq(paymentConnections.provider, 'yoco')));
