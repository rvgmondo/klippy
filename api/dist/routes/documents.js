...
export async function documentRoutes(app) {
    ...
    app.get('/api/v1/collections', async (req) => {
        ...
        if (folderIds.length) {
            ...
            for (const c of cRows)
                if (c.folderId != null && normalizePhone(c.phone))
                    phoned.add(c.folderId);
        }
        const items = rows
            .map((r) => ({
        ...
        for (const group of groups.values()) {
            ...
            if (first.folderId) {
                ...
                if (st) {
                    ...
                    if (pdf2)
                        attachment = { filename: pdf2.filename, content: pdf2.buffer };
                }
            }
    ...
    app.post('/api/v1/statements/:id/email', async (req, reply) => {
        ...
        if (!client.billingEmail)
            return reply.code(400).send({ error: 'This client has no billing email. Add one on the client first.' });
    ...
    app.post('/api/v1/documents', async (req, reply) => {
        ...
        if (d.type === 'invoice' && d.fromTime) {
            ...
            const subtree2 = new Set([ftFolder]);
            let grew2 = true;
            while (grew2) {
                grew2 = false;
                for (const f of allFolders2) {
                    if (f.parentId != null && subtree2.has(f.parentId) && !subtree2.has(f.id)) {
                        subtree2.add(f.id);
                        grew2 = true;
                    }
                }
