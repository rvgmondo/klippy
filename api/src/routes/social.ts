...
async function withdrawApproval(
  accountId: number,
  post: { id: number; status: string; approvedAt: Date | null; approvalToken: string | null },
): Promise<boolean> {
  ...
  await db.insert(socialPublishLog).values(withTenant(accountId, {
    postId: post.id, level: 'warn' as const,
 ...
 */
async function setTargets(accountId: number, postId: number, businessId: number, networks: Network[]): Promise<void> {
    ...
    if (have.has(n)) {
      ...
      await db.update(socialPostTargets).set({ socialAccountId: accountFor.get(n) ?? null })
        .where(tenantWhere(socialPostTargets, accountId,
          eq(socialPostTargets.postId, postId), eq(socialPostTargets.network, n)));
      continue;
    }
