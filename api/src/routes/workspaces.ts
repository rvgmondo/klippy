...
export async function workspaceRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/workspaces/switch', async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'accountId required.' });

    const m = await getMembership(parsed.data.accountId, userId);
    if (!m || !m.isActive) return reply.code(403).send({ error: 'You are not a member of that workspace.' });
  ...
  app.post('/api/v1/workspaces', async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Give the workspace a name.' });
  ...
  app.post('/api/v1/workspaces/:id/leave', async (req, reply) => {
    ...
    if (!m) return reply.code(404).send({ error: 'You are not in that workspace.' });
    if (m.role === 'owner') return reply.code(400).send({ error: "An owner can't leave their own workspace." });
