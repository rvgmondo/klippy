...
type Outcome = 'awaiting' | 'approved' | 'changes' | 'closed';
...
function whenLabel(at: Date | null, timeZone: string): string | null {
  if (!at) return null;
  try {
...
export async function socialApprovalRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/approve/:token', async (req, reply) => {
    ...
    if (!TOKEN_RE.test(token)) return reply.code(404).send({ error: 'This link is not valid.' });
    ...
    if (!post) return reply.code(404).send({ error: 'This link is not valid.' });
  ...
  app.post('/api/v1/approve/:token', async (req, reply) => {
    ...
    await db.insert(socialPublishLog).values(withTenant(accountId, {
      postId: post.id,
      level: decision === 'approve' ? ('info' as const) : ('warn' as const),
