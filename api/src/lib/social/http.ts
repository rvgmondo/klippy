import { SocialApiError, type Network } from './types.js';
...
export interface HttpOptions {
  ...
  /** Sent as form-encoded, which is what the Graph API expects for writes. */
...
export function scrubUrl(url: string): string {
  try {
    ...
    for (const key of [...u.searchParams.keys()]) {
      if (/token|secret|code|signature|appsecret/i.test(key)) u.searchParams.set(key, '[redacted]');
    }
    return u.toString();
  } catch {
 ...
 */
export function classifyMetaError(body: unknown, status: number): { message: string; retryable: boolean; code: string } {
  ...
  if (err.is_transient === true) return { message, retryable: true, code: label };
  ...
  if (RETRY_SUBCODES.has(sub)) return { message, retryable: true, code: label };
  ...
  if (sub === 2207020 || sub === 2207042) return { message, retryable: true, code: label };
