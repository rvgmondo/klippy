...
async function subtreeIds(accountId: number, rootId: number): Promise<number[]> {
...
export async function trashRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/trash', async (req) => {
    ...
    const daysLeft = (d: Date) => Math.max(0, 30 - Math.floor((Date.now() - d.getTime()) / 86_400_000));
    const items = [
      ...fRows
        .filter((f) => (f.parentId == null || !deletedFolderIds.has(f.parentId)) && canSee(f.businessId))
        .map((f) => ({
          kind: 'folder' as const, id: f.id, name: f.name,
      ...
      ...bRows
        ...
        .map((b) => ({
          kind: 'board' as const, id: b.id, name: b.name,
          detail: b.folderName,
  ...
  app.post('/api/v1/trash/restore', async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'kind and id required.' });
    ...
    if (kind === 'folder') {
      ...
      if (!f || !f.deletedAt) return reply.code(404).send({ error: 'Not in the trash.' });
      if (!(await assertMaybeBusiness(req, reply, f.businessId))) return;
    ...
    if (!b || !b.deletedAt) return reply.code(404).send({ error: 'Not in the trash.' });
    if (!(await assertMaybeBusiness(req, reply, b.businessId ?? null))) return;
    await db.update(boards).set({ deletedAt: null })
