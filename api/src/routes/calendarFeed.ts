...
const dateStamp = (d: Date) => d.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}Z$/, 'Z');
...
export async function calendarFeedRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/public/calendar/:accountId/:userId.ics', async (req, reply) => {
    const p = req.params as { accountId: string; userId: string };
    ...
    const bizCond = (col: AnyMySqlColumn): SQL | undefined => {
    ...
    const cards = await db.select({
      ...
      .where(and(
        ...
        isNotNull(tasks.dueDate), gte(tasks.dueDate, from), lte(tasks.dueDate, to),
        or(eq(tasks.assignedTo, userId), isNull(tasks.assignedTo)),
        bizCond(folders.businessId),
      ));
    ...
    const evs = await db.select().from(calendarEvents)
      .where(and(
        ...
        gte(calendarEvents.startAt, evStart), lte(calendarEvents.startAt, evEnd),
        bizCond(calendarEvents.businessId),
      ));
    ...
    for (const c of cards) {
      ...
      lines.push(
        'BEGIN:VEVENT',
        `UID:klippy-task-${c.id}@klippy`,
        `DTSTAMP:${now}`,
        `DTSTART;VALUE=DATE:${day}`,
        `SUMMARY:${escText(`${c.title}${c.boardName ? ` (${c.boardName})` : ''}`)}`,
        'END:VEVENT',
      );
    }
    for (const e of evs) {
      ...
      if (e.allDay) {
        ...
        if (e.endAt) lines.push(`DTEND:${dateStamp(e.endAt)}`);
      }
