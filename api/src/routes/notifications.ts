...
export async function notificationRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/notifications', async (req) => {
    ...
    const items = await db.select({
      ...
      .limit(30);
    const [row] = await db.select({ n: sql<number>`COUNT(*)` }).from(notifications)
      .where(tenantWhere(notifications, accountId,
  ...
  app.post('/api/v1/notifications/read', async (req, reply) => {
    ...
    await db.update(notifications).set({ readAt: new Date() })
      .where(and(
        tenantWhere(notifications, accountId, eq(notifications.userId, userId), isNull(notifications.readAt)),
        parsed.data.all ? undefined : inArray(notifications.id, parsed.data.ids!),
      ));
    return { ok: true };
  });
}
