...
export async function searchRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/search', async (req, reply) => {
    ...
    if (!q.success) return reply.send({ tasks: [], clients: [], deals: [], contacts: [], documents: [], offerings: [] });
    ...
    const dealRows = await db.select({
      ...
      .where(tenantWhere(deals, accountId,
        or(like(deals.title, term), like(deals.company, term), like(deals.contactName, term)),
    ...
    const contactRows = await db.select({
      ...
      .where(tenantWhere(contacts, accountId,
        or(like(contacts.name, term), like(contacts.company, term), like(contacts.email, term)),
        await businessScope(req, contacts.businessId),
      ))
      .limit(8);
