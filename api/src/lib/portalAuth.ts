...
export async function portalContext(token: string | undefined): Promise<PortalContext | null> {
   ...
   */
  if (!client || client.isArchived || client.deletedAt) return null;
 ...
 */
export async function issueLoginTokenForUser(portalUserId: number): Promise<{ raw: string; user: typeof portalUsers.$inferSelect } | null> {
 ...
 */
export async function issueLoginTokensForEmail(email: string): Promise<Array<{ raw: string; user: typeof portalUsers.$inferSelect }>> {
  ...
  const issued: Array<{ raw: string; user: typeof portalUsers.$inferSelect }> = [];
  for (const r of rows) {
    const one = await issueLoginTokenForUser(r.id);
    if (one) issued.push(one);
  }
  return issued;
}
...
export async function passwordLogin(email: string, password: string): Promise<typeof portalUsers.$inferSelect | null> {
  ...
  const now = Date.now();
  let matched: typeof portalUsers.$inferSelect | null = null;
  let compared = false;
  for (const u of rows) {
    ...
    if (u.lockedUntil && u.lockedUntil.getTime() > now) continue;
    if (!u.passwordHash) continue;
    compared = true;
    if (await verifyPassword(password, u.passwordHash).catch(() => false)) { matched = u; break; }
  }
...
export function generatePortalPassword(): string {
  ...
  const word = (i: number) => PW_WORDS[bytes[i]! % PW_WORDS.length]!;
