...
export async function buildAccountExport(accountId) {
    ...
    const [biz, folderRows, boardRows, columnRows, taskRows, timeRows, contactRows, dealRows, dealActivityRows, docRows, lineRows, paymentRows, offeringRows, subscriptionRows, expenseRows, subtaskRows, commentRows, fileRows, labelRows, taskLabelRows, teamRows, teamMemberRows, boardTeamRows, eventRows, storageRows, hostingRows, portalRows, noteRows, focusRows, peopleRows, salesRows, recurringRows,] = await Promise.all([
        ...
        db.select({ id: timeEntries.id, taskId: timeEntries.taskId, userId: timeEntries.userId, startTime: timeEntries.startTime, durationSeconds: timeEntries.durationSeconds, billedDocumentId: timeEntries.billedDocumentId }).from(timeEntries).where(own(timeEntries)),
        ...
        db.select({ id: payments.id, documentId: payments.documentId, amount: payments.amount, paidOn: payments.paidOn, method: payments.method, note: payments.note, pfPaymentId: payments.pfPaymentId, feeAmount: payments.feeAmount, netAmount: payments.netAmount }).from(payments).where(own(payments)),
