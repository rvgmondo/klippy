...
const pending = new Map<string, {
  accountId: number; businessId: number; userId: number; network: Network;
  userToken: string; refreshToken: string | null; expiresAt: Date | null;
...
const redirectFor = (network: string) =>
  `${(process.env.APP_URL ?? appUrl() ?? '').replace(/\/+$/, '')}/api/v1/social/connect/${network}/callback`;

/** Back to the app with a message, since a callback has nowhere else to land. */
function bounce(reply: unknown, message: string, ok = false): void {
  const base = (appUrl() ?? '').replace(/\/+$/, '');
  const q = new URLSearchParams({ v: 'social', [ok ? 'connected' : 'connectError']: message });
  (reply as { redirect: (url: string) => void }).redirect(`${base}/?${q}`);
}

export async function socialConnectRoutes(app: FastifyInstance) {
  // ---- 1. Start, signed in -------------------------------------------------------
  app.get('/api/v1/social/connect/:network/start', { preHandler: app.requireAuth }, async (req, reply) => {
    const { accountId, userId } = authOf(req);
    const network = (req.params as { network?: string }).network as Network;
    const adapter = adapterFor(network);
    if (!adapter) return reply.code(400).send({ error: `Klippy cannot connect ${network} yet.` });
    const q = z.object({ businessId: z.coerce.number().int().positive() }).safeParse(req.query);
    if (!q.success) return reply.code(400).send({ error: 'Which business is this for?' });
    if (!(await assertBusinessAccess(req, reply, q.data.businessId, 'admin'))) return;
  ...
  app.get('/api/v1/social/connect/:network/callback', async (req, reply) => {
    ...
    if (!q.success) return bounce(reply, 'That connection link was malformed.');
    ...
    if (!q.data.code || !q.data.state) return bounce(reply, 'That connection link was incomplete.');
    ...
    try {
      ...
      pending.set(handoff, {
        ...
        refreshToken: isLinkedIn ? ((exchanged as { refreshToken?: string | null }).refreshToken ?? null) : null,
  ...
  app.get('/api/v1/social/connect/pending', { preHandler: app.requireAuth }, async (req, reply) => {
    ...
    if (!q.success) return reply.code(400).send({ error: 'Bad handoff.' });
    ...
    for (const externalId of parsed.data.externalIds) {
      ...
      const existing = await db.select({ id: socialAccounts.id }).from(socialAccounts)
        .where(tenantWhere(socialAccounts, accountId,
          eq(socialAccounts.businessId, row.businessId),
          eq(socialAccounts.network, network),
          eq(socialAccounts.externalId, externalId))).limit(1);
