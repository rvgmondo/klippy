...
async function assertColumnInAccount(accountId: number, boardId: number, columnId: number): Promise<boolean> {
    ...
    await db.transaction(async (tx) => {
      ...
      const siblings = await tx.select({ id: tasks.id }).from(tasks)
        .where(tenantWhere(tasks, accountId, and(
          eq(tasks.columnId, parsed.data.columnId), eq(tasks.isArchived, false), ne(tasks.id, id))))
