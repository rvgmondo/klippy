...
export async function hostingRoutes(app: FastifyInstance) {
  ...
  const write = async (
    ...
    if (d.tempDomainPattern !== undefined) patch.tempDomainPattern = d.tempDomainPattern || null;
  ...
  app.delete('/api/v1/hosting/accounts/:id', async (req, reply) => {
    ...
    if (role === 'member') return reply.code(403).send({ error: 'Only admins can remove hosting records.' });
    const id = intId(req);
    if (!id) return reply.code(400).send({ error: 'Bad id.' });
    ...
    }).catch(() => { /* the removal still stands */ });
