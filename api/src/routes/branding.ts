...
async function sendImage(reply: FastifyReply, stored: string | null) {
  ...
  if (ext === '.svg') reply.header('Content-Disposition', 'attachment');
  if (!reply.getHeader('Cache-Control')) reply.header('Cache-Control', 'private, max-age=300');
 ...
 */
export async function publicBrandingRoutes(app: FastifyInstance) {
  app.get('/api/v1/public/logo/:kind/:id', async (req, reply) => {
    ...
    if (!verifyLogoToken(kind, numId, token)) return reply.code(404).send({ error: 'Not found.' });
