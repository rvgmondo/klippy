...
export const linkedinConfigured = () => !!(clientId() && clientSecret());
 ...
 */
function linkedinError(status, body) {
    const b = body;
    const message = b?.message ?? `LinkedIn returned HTTP ${status}`;
    const retryable = status === 429 || status >= 500;
    const authGone = status === 401 || status === 403;
    return new SocialApiError('linkedin', authGone ? `${message} The LinkedIn connection needs to be re-authorised.` : message, {
        retryable,
...
async function call(url, init) {
    ...
    try {
        ...
        throw new SocialApiError('linkedin', aborted ? 'LinkedIn did not answer in time.' : 'Could not reach LinkedIn.', { retryable: true, code: aborted ? 'timeout' : 'network' });
    }
    finally {
        clearTimeout(timer);
    }
    ...
    if (!res.ok)
        throw linkedinError(res.status, body);
    return { body: body, headers: res.headers };
}
// ---- OAuth ---------------------------------------------------------------------
/** LI-AUTH-01. */
export function linkedinAuthUrl(state, redirectUri) {
 ...
 */
export async function linkedinExchangeCode(code, redirectUri) {
 ...
 */
export async function linkedinListOrganisations(token) {
    const { body } = await call(`${REST}/organizationAcls?${new URLSearchParams({
    ...
    for (const urn of urns) {
        ...
        let logo = null;
        try {
            const { body: org } = await call(`${REST}/organizations/${id}`, { method: 'GET', headers: headers(token), token });
 ...
 */
async function uploadImage(token, owner, storageKey) {
    const { body } = await call(`${REST}/images?action=initializeUpload`, {
        method: 'POST',
    ...
    if (!uploadUrl || !imageUrn) {
        throw new SocialApiError('linkedin', 'LinkedIn did not return an upload URL.', { retryable: true, code: 'no-upload-url' });
    }
    const size = await storage().size(storageKey);
    if (size == null) {
        throw new SocialApiError('linkedin', 'The image file is missing from storage.', { retryable: false, code: 'missing-file' });
    }
    const res = await fetch(uploadUrl, {
        ...
        body: storage().createReadStream(storageKey),
...
async function publishLinkedIn(creds, post, media) {
    ...
    if (!urn) {
        throw new SocialApiError('linkedin', 'LinkedIn accepted the post but returned no id.', { retryable: false, code: 'no-restli-id' });
    }
...
export const linkedinAdapter = {
    ...
    async publish(creds, post, media) {
        ...
        if (!linkedinConfigured()) {
            throw new SocialApiError('linkedin', 'LINKEDIN_CLIENT_ID and LINKEDIN_CLIENT_SECRET are not set on the server.', { retryable: false, code: 'not-configured' });
        }
