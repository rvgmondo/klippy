...
export interface TemplateCard { title: string; description?: string; column?: number }
export interface TemplateColumn { name: string; color: string; isDoneColumn?: boolean }
...
const TODO_DOING_DONE: TemplateColumn[] = [
...
export const boardTemplate = (key: string) => BY_KEY.get(key);

/** Columns for a template, defaulting to the ordinary three. */
export const columnsFor = (t: BoardTemplate): TemplateColumn[] => t.columns ?? TODO_DOING_DONE;
