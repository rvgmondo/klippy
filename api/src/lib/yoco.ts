...
export interface YocoMoney { amount: number; currency: string }

export interface YocoPayment {
  id: string;
  status: string;
  createdAt: string;
  /** card_machine, checkout, invoice, payment_link, other, ... */
  source: string | null;
  cardMachineId: string | null;
  currency: string;
  /** All in MAJOR units (rand), already converted from their cents. */
  gross: number;
  fee: number;
  tip: number;
  refunded: number;
...
export type YocoResult<T> =
  | { ok: true; data: T }
  | { ok: false; message: string; retryable: boolean };

/** Cents to rand. Yoco sends integers; everything downstream is decimal money. */
const major = (v: unknown): number => {
  const n = typeof v === 'number' ? v : Number((v as { amount?: unknown } | null)?.amount);
  return Number.isFinite(n) ? n / 100 : 0;
};
const currencyOf = (v: unknown, fallback = 'ZAR'): string => {
  const c = (v as { currency?: unknown } | null)?.currency;
...
export async function listPayments(
  ...
  for (const raw of rows as Record<string, unknown>[]) {
    ...
    const fee = fees.reduce((sum: number, f: unknown) => sum + Math.abs(major(f)), 0);
  ...
  const meta = (root.meta ?? root.pagination ?? root) as Record<string, unknown>;
 ...
 */
export async function testKey(secret: string): Promise<YocoResult<{ payments: number }>> {
