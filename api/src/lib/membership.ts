 ...
 */
export async function getMembership(accountId: number, userId: number) {
...
export async function isActiveMember(accountId: number, userId: number): Promise<boolean> {
  const m = await getMembership(accountId, userId);
  return !!m && m.isActive;
}

/** Every workspace this person can open, with their role in each. */
export async function workspacesFor(userId: number) {
  return db.select({
...
export async function membersOf(accountId: number) {
  return db.select({
    id: users.id,
    name: users.name,
    email: users.email,
...
export async function addMember(accountId: number, userId: number, role: Role = 'member') {
