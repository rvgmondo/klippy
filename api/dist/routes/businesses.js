...
export async function businessRoutes(app) {
    ...
    app.patch('/api/v1/businesses/:id', async (req, reply) => {
        ...
        for (const k of ['brandName', 'bizAddress', 'bizTaxNumber', 'bizRegNumber', 'bankDetails', 'invoiceFooter']) {
            if (patch[k] === '')
                patch[k] = null;
        }
    ...
    app.delete('/api/v1/businesses/:id', async (req, reply) => {
        ...
        if (Number(countRow?.n ?? 0) <= 1)
            return reply.code(400).send({ error: 'You need at least one business.' });
    ...
    app.patch('/api/v1/businesses/:id/email', async (req, reply) => {
        ...
        if (!(await assertBusinessAccess(req, reply, id, 'admin')))
            return;
        const em = (max) => z.string().trim().max(max).optional().or(z.literal(''));
        ...
        const blankToNull = (v) => (v === undefined ? undefined : (v === '' ? null : v));
        const patch = {
        ...
        if (d.smtpPort !== undefined)
            patch.smtpPort = d.smtpPort;
        if (d.smtpSecure !== undefined)
            patch.smtpSecure = d.smtpSecure;
        if (d.smtpPass)
            patch.smtpPassEnc = encryptSecret(d.smtpPass);
        else if (d.smtpPass === '')
