    ...
    for (const target of targets) {
        ...
        if (!target.socialAccountId) {
            const connected = await db.select().from(socialAccounts)
                .where(tenantWhere(socialAccounts, post.accountId, eq(socialAccounts.businessId, post.businessId), eq(socialAccounts.network, network), eq(socialAccounts.status, 'connected')));
 ...
 */
async function manualReason(post, boundId, account, network) {
    const adapter = adapterFor(network);
    ...
    const [row] = await db.select({ n: sql `count(*)` }).from(socialAccounts)
        .where(tenantWhere(socialAccounts, post.accountId, eq(socialAccounts.businessId, post.businessId), eq(socialAccounts.network, network), eq(socialAccounts.status, 'connected')));
    if (Number(row?.n ?? 0) > 1) {
