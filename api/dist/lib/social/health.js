...
import { SocialApiError, NETWORK_LABEL } from './types.js';
...
export async function checkAccount(row) {
    ...
    if ((network === 'instagram' || network === 'facebook') && !metaConfigured()) {
        ...
        return { ok: false, status: row.status, message: 'The Meta app is not set up on this server.' };
    }
    let token;
    try {
    ...
    try {
        ...
        await httpJson(`${GRAPH}/${row.externalId}?${new URLSearchParams({ fields: 'id', access_token: token })}`, { network, timeoutMs: 15_000 });
        ...
        if (err instanceof SocialApiError) {
            ...
            if (err.retryable) {
                return { ok: false, status: row.status, message: `Could not reach ${label} just now.` };
            }
...
async function record(row, status, message) {
 ...
 */
export async function runSocialTokenCheck() {
