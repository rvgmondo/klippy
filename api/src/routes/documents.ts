...
function computeTotals(
  ...
  depositType: 'none' | 'percent' | 'amount' = 'none', depositValue = 0,
) {
  ...
  // than the invoice.
  let depositAmount = 0;
  if (depositType === 'percent') depositAmount = r(total * (Math.min(depositValue, 100) / 100));
  else if (depositType === 'amount') depositAmount = r(Math.min(depositValue, total));
...
export async function documentRoutes(app: FastifyInstance) {
  ...
  app.put('/api/v1/documents/:id', async (req, reply) => {
     ...
     */
    const [everPaid] = await db.select({ n: sql<number>`COUNT(*)` }).from(payments)
  ...
  app.delete('/api/v1/payments/:id', async (req, reply) => {
    ...
    if (!pay) return reply.code(404).send({ error: 'Not found.' });
    ...
    if (doc.type === 'invoice' && doc.status === 'paid'
      ...
      if (reopened) {
        ...
        if (again.flipped) reopened = false;
      }
    }
