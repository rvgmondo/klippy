...
export interface IssuedInvite { id: number; raw: string }
...
export async function issueInvitation(
  accountId: number, email: string, role: 'admin' | 'member', createdBy: number,
): Promise<IssuedInvite | null> {
  const now = new Date();
  const [recent] = await db.select({ createdAt: invitations.createdAt })
    .from(invitations)
    .where(and(eq(invitations.accountId, accountId), eq(invitations.email, email),
...
export async function sendInvitationEmail(
  accountId: number, email: string, raw: string, invitedByName: string | null,
): Promise<void> {
  ...
  ).catch(() => { /* an invitation that fails to send is still revocable and re-sendable */ });
}

export type AcceptResult =
  | { ok: true; accountId: number; role: 'admin' | 'member' }
 ...
 */
export async function acceptInvitation(rawToken: string, userId: number): Promise<AcceptResult> {
  ...
  if (!inv) return { ok: false, message: 'That invitation is not valid any more. Ask for a new one.' };
