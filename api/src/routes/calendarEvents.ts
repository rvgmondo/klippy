...
export async function calendarEventRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/calendar-events', async (req, reply) => {
    ...
    const rows = await db.select({
      ...
      .where(tenantWhere(calendarEvents, accountId,
        gte(calendarEvents.startAt, new Date(`${q.data.from}T00:00:00.000Z`)),
        lte(calendarEvents.startAt, new Date(`${q.data.to}T23:59:59.999Z`)),
        q.data.businessId ? eq(calendarEvents.businessId, q.data.businessId) : undefined,
  ...
  app.post('/api/v1/calendar-events', async (req, reply) => {
    ...
    if (!startAt) return reply.code(400).send({ error: 'Start time is not a valid date.' });
  ...
  app.patch('/api/v1/calendar-events/:id', async (req, reply) => {
    ...
    if (parsed.data.startAt !== undefined) {
      ...
      if (!s) return reply.code(400).send({ error: 'Start time is not a valid date.' });
      patch.startAt = s;
    }
    if (parsed.data.endAt !== undefined) {
      patch.endAt = parsed.data.endAt ? parseWhen(parsed.data.endAt) : null;
      if (parsed.data.endAt && !patch.endAt) return reply.code(400).send({ error: 'End time is not a valid date.' });
    }
    const start = (patch.startAt as Date) ?? own.startAt;
    if (patch.endAt && (patch.endAt as Date) < start) {
