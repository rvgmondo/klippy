...
function computeTotals(lines, taxRate, discountType = 'none', discountValue = 0, currency = DEFAULT_CURRENCY) {
    ...
    const r = (n) => roundMoney(n, currency);
    ...
    if (discountType === 'percent')
        discountAmount = r(subtotal * (Math.min(discountValue, 100) / 100));
...
export async function documentRoutes(app) {
    ...
    app.get('/api/v1/statements/:id', async (req, reply) => {
        ...
        if (!(await assertMaybeBusiness(req, reply, client.businessId, 'viewer')))
            return;
        const allDocs = await db.select({
            ...
            .where(tenantWhere(documents, accountId, eq(documents.folderId, folderId), inArray(documents.type, ['invoice', 'credit_note']), ne(documents.status, 'void'), ne(documents.status, 'draft'), // a draft has not been issued to anyone
        from ? gte(documents.issueDate, from) : undefined, to ? lte(documents.issueDate, to) : undefined));
        ...
        const payRows = docIds.length
            ? await db.select({
                ...
                .where(tenantWhere(payments, accountId, inArray(payments.documentId, docIds), from ? gte(payments.paidOn, from) : undefined, to ? lte(payments.paidOn, to) : undefined))
            : [];
        const numberOf = new Map(docs.map((d) => [d.id, d.number]));
        const entries = [
            ...docs.map((d) => d.type === 'invoice'
