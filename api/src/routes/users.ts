...
async function activeAdminCount(accountId: number, excludeUserId?: number): Promise<number> {
  const [row] = await db.select({ c: sql<number>`COUNT(*)` }).from(users)
    .where(tenantWhere(users, accountId, and(
...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/users', async (req, reply) => {
    ...
    if (dupe) return reply.code(409).send({ error: 'That email is already in use.' });
  ...
  app.patch('/api/v1/users/:id', async (req, reply) => {
    ...
    if (!target) return reply.code(404).send({ error: 'Person not found.' });
    ...
    if (parsed.data.password) { patch.passwordHash = await hashPassword(parsed.data.password); patch.failedAttempts = 0; patch.lockedUntil = null; }
